...
# Put this file next to the script (or change the path):
# mongo_uri.py should contain:  MONGO_URI = "mongodb+srv://..."
...
def get_mongo_collection(uri: str):
    ...
    # event_id unique ensures duplicates are rejected/upserted safely
    try:
...
def main():
    ...
    with INPUT_FILE.open("r", encoding="utf-8", errors="replace") as log_file, \
        ...
        while True:
            ...
            # Debug: show that we are seeing new lines at all
            print("[RAW]", line)
            ...
            # Add dedupe id (doesn't change your file format materially; still valid JSON)
            event["event_id"] = compute_event_id(event)

            # ---- Keep your current functionality intact ----
            out_file.write(json.dumps(event) + "\n")
            out_file.flush()

            # ---- Add Mongo append behavior ----
            if mongo_enabled:
                try:
                    ...
                    # If unique index exists and a duplicate slips through, ignore it
                    pass
                except Exception as e:
